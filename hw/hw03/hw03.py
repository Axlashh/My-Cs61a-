...
def num_eights(pos):
    ...
    def helper(x,po):
        if po==0:
            return x
        if po%10 == 8:
            return helper(x+1,po//10)
        else:
            return helper(x,po//10)
    return helper(0,pos)
...
def pingpong(n):
    ...
    def ise(num):
        if num==0:
            ...
            return ise(num//10)
    def helper(th,change,num):
        if th == n:
            return num
        if th%8==0 or ise(th):
            return helper(th+1,-change,num-change)
        else:
            return helper(th+1,change,num+change)
    return helper(1,1,1)
...
def count_coins(change):
    ...
    def helper(chan,i):
        ...
        if i==1 or chan==0:
            return 1
        return helper(chan-i,i)+helper(chan,next_smaller_coin(i))
