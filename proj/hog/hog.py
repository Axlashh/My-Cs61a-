...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while i<=10:
        ...
        temp = averaged_roll_dice(i,dice)
        if max<temp:
            max = temp
            ans = i
        i += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if tail_points(opponent_score) >= threshold:
        return 0
    if perfect_square(tail_points(opponent_score)+score):
        if next_perfect_square(tail_points(opponent_score)+score)-score >= threshold:
            return 0
    return num_rolls
    # END PROBLEM 11
