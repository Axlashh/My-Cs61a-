...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        def check(n):
            ...
            if (fi>='a' and fi<='z') or (fi>='A' and fi<='Z'):
                return check(n.rest)
            else:
                return False
        if not check(signature.rest):
            raise SchemeError('Wrong type of formals')
        tem = LambdaProcedure(signature.rest,expressions.rest,env)
        env.define(signature.first,tem)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    tem = scheme_eval(expressions.first,env)
    if is_scheme_false(tem) or expressions.rest == nil:
        return tem
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    tem = scheme_eval(expressions.first,env)
    if is_scheme_true(tem) or expressions.rest == nil:
        return tem
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil: 
        ...
        validate_form(k,2,2)
        if type(k.first) != str:
            raise SchemeError('arguments invalid')
        names = Pair(k.first,names)
        vals = Pair(scheme_eval(k.rest.first,env),vals)
