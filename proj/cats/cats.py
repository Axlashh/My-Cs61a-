...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def func(lines):
        new_lines = split(lower(remove_punctuation(lines)))
        for i in new_lines:
            for j in topic:
                if i == j:
                    return True
...
def accuracy(typed, source):
    ...
    if len(typed_words)>len(source_words):
        leng = len(source_words)
    else:
        leng = len(typed_words)
    for i in range(leng):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        ...
        if diff_function(typed_word,i,limit)<=limit:
            if diff_function(typed_word,i,limit)<low_dif:
                low_dif = diff_function(typed_word,i,limit)
                ind = i
    return ind
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    leng = min(len(typed),len(source))
    def helper(i,ans):
        if ans>limit:
            return ans
        if i == leng:
            return ans+abs(len(source)-len(typed))
        if typed[i] != source[i]:
            ans += 1
